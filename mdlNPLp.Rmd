...
```{r}
formula <- NPLp ~ iloUnempl + I(iloUnempl^2) + perSIDinNGDP + indexInstitute + indexBureau + indexPayment+ LNPLp + indexDemocracy + I(GREV*100/NGDP) + I(GEXP*100/NGDP) + Tax+ bvQ1 + bvQ2 + bvQ3 
...
```{r}
tab <- matrix(c(c(mdlRB$coefficients, rep(NA,3)), cplRB$coefficients), ncol=2)
colnames(tab) <- c("OLS", "Copula")
...
```{r}
formula <- NPLp ~ perSIDinNGDP + I(GREV*100/NGDP) + I(GEXP*100/NGDP) +WSGDP + Unempl + indexBureau  + indexDemocracy + indexInstitute + indexPayment + bvQ1 + bvQ2 +bvQ3
